    ...
    {questionStep === 'intro' && (
        <>
          ...
          <p style={{ marginBottom: "20px" }}>
            Этот инструмент не даёт прямого ответа. Он помогает тебе остановиться, почувствовать, услышать себя — без давления извне.
Он создан для внутренних решений, которые влияют на твою жизнь дольше, чем просто день. Лучше всего — понаблюдать за вопросом в тишине, а может — в медитации.
Это не подходит для ситуаций, где нужно срочно действовать — например, по здоровью или оплатам.<br />
Примеры подходящих тем:<br />
– сменить работу или переехать в другую страну
– завести кота или начать новый проект<br />
Сейчас попробуй честно почувствовать, к какой сфере относится твой вопрос. 
          </p>
        ...
        <>
         ...
         <div className="button-column">
          ...
          <button onClick={() => { setQueryResult(generateQueryResult(0)); setQuestionStep('result'); }}>
            Про безопасность, дом и деньги<br />
            <span style={{ fontSize: "80%" }}>Переезд, Увольнение, Финансовая опора</span>
          </button>
        
          {/* 🟠 Свадхистхана */}
          <button onClick={() => { setQueryResult(generateQueryResult(1)); setQuestionStep('result'); }}>
            Про чувства, удовольствие и тело<br />
            <span style={{ fontSize: "80%" }}>Новые отношения, Беременность, Переезд на остров</span>
          </button>
        
          {/* 🟡 Манипура */}
          <button onClick={() => { setQueryResult(generateQueryResult(2)); setQuestionStep('result'); }}>
            Про цель, силу и развитие<br />
            <span style={{ fontSize: "80%" }}>Открыть бизнес, Выйти из зависимости, Добиться роста</span>
          </button>
        
          {/* 🟢 Анахата */}
          <button onClick={() => { setQueryResult(generateQueryResult(3)); setQuestionStep('result'); }}>
            Про любовь, близость и связь<br />
            <span style={{ fontSize: "80%" }}>Закрыть прошлое, Восстановить контакт, Открыться чувствам</span>
          </button>
        
          {/* 🔵 Вишудха */}
          <button onClick={() => { setQueryResult(generateQueryResult(4)); setQuestionStep('result'); }}>
            Про творчество, голос и проявление<br />
            <span style={{ fontSize: "80%" }}>Запустить блог, Выступать, Делать личный бренд</span>
          </button>
        
          {/* 🟣 Аджна */}
          <button onClick={() => { setQueryResult(generateQueryResult(5)); setQuestionStep('result'); }}>
            Про интуицию, сценарий и предназначение<br />
            <span style={{ fontSize: "80%" }}>Найти себя, Уйти с работы, Перепридумать путь</span>
          </button>
        
          {/* ⚪ Сахасрара */}
          <button onClick={() => { setQueryResult(generateQueryResult(6)); setQuestionStep('result'); }}>
            Вопрос про доверие, тишину и смысл<br />
            <span style={{ fontSize: "80%" }}>Уехать в ретрит, Отказаться от контроля, Довериться потоку</span>
          </button>
        </div>
